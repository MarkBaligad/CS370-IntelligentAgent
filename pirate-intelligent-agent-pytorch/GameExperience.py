...
class GameExperience(object):
    ...
    # target_model = target network for stable Q-learning (optional)
    ...
    def __init__(self, model, target_model=None, max_memory=100, discount=0.95):
        self.model = model
    ...
    def get_data(self, data_size=10):
        ...
        for i, j in enumerate(np.random.choice(range(mem_size), data_size, replace=False)):
            ...
            # Use target network for stable Q-value predictions
            Q_sa = np.max(self.target_model.predict(envstate_next)[0])
            if game_over:
